...
int main(void) {
    ...
    /* Préparation : On se place dans le dossier multi/ (parent) pour avoir les fichiers sources */
    /* Mais le binaire test_multithreading s'exécute depuis test/ */
    
    /* SCÉNARIO 1 : Plusieurs PUT simultanés du MÊME fichier */
    printf("━━━ SCÉNARIO 1 : 3 PUT simultanés sur 'server.c' ━━━\n");
    pthread_t t1[3];
    ParametresTest p1[3];
    /* On crée des copies locales dans test/ pour que le client les trouve */
    system("cp ../server.c ."); 
    for (int i = 0; i < 3; i++) {
        p1[i].id_thread = i + 1;
    ...
    system("cp ../client.c ."); /* Prépare le fichier pour le PUT */
